...
def verify_login(user: LoginUserSchema, db: Session):
    return db.query(User, Token).filter(User.id == Token.user_id).filter(User.username == user.username).filter(User.password == user.password).filter(Token.token == user.token).first()